...
for i=1:nx
 for j=2:ny-1
     y=j-(ny+1)/2;
     if y<0;
         T_air(i,j) = mask(i,j)*(273.15+37.5*cos(pi*y/theta)-8.5);
     else
         T_air(i,j) = mask(i,j)*(273.15+29.0*cos(pi*y/theta)-0.0);
     end
 end
end
...
for i=1:nx % Southern Ocean
 for j=2:19
     y=j-(ny+1)/2;
     emp(i,j) = -empconst*mask(i,j)*(cos(1.9*pi*abs(2*(y-3)/theta1)^0.75))*(2.6*cos(0.8*pi*abs((y-3)/theta1)^1.9))+0.2*10^-8;
 end
 for j=16:25
     y=j-(ny+1)/2;
...
for i=1:30 % Atlantic
 for j=26:ny-1
     y=j-(ny+1)/2;
     emp(i,j) = -empconst*mask(i,j)*(cos(2.2*pi*abs(2*(y-2)/theta1)^0.6))*(2*cos(0.75*pi*abs(y/theta1)^1));
    if y<0
